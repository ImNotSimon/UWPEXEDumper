...
std::uint32_t __stdcall DumperThread(void* DLLHandle)
{
	...
	IPC::PushMessage(L"UWP EXE Dumper Build date(%ls : %ls)\n", WIDEIFY(__DATE__), WIDEIFY(__TIME__));
	IPC::PushMessage(L"\t-https://github.com/ImNotSimon/UWPEXEDumper\n");
	...
	{
		if (fs::is_regular_file(Entry.path()) && Entry.path().extension() == ".exe")
		{
